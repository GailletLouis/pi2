...
nbrInputNotUsed = 0;
xgb = NULL
wavCSVDataFile = 'my_voice.csv'
csvDatabaseFile = 'voice.csv'
recordingData = NULL

#' Echo the parameter that was sent in
#' @get /predictGender
function(){
    recordingData <<- dataset[index,]
    pred <- predict(xgb,data.matrix(recording))
    gender = ifelse(pred > 1, "Male", "Female")
    gender
}
...
#' @get /addNewInput
function(gender){
  addNewInputToDatabase(genderLabel = gender)
  nbrInputNotUsed
}
...
writeDataFromWav <- function(){
  
  # Load data
  soundFile <- processFolder('soundFile')
  ...
  write.csv(soundFile, file = wavCSVDataFile, row.names=F)
}
...
#Training the model
trainModel <- function(){
  dataset <- read.csv(csvDatabaseFile)
...
addNewInputToDatabase <- function(genderLabel){
  if(!is.null(recordingData)){
    recordingData$label <- genderLabel
    write.csv(recordingData, file = 'csvDatabaseFile.csv', row.names=F, append = TRUE)
    recordingData <<- NULL
    nbrInputNotUsed <<- nbrInputNotUsed + 1
  }
}